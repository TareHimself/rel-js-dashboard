    ...
    useEffect(() => {
        const decideCloseMenu = (ev: MouseEvent) => {
            ...
            if (dropdown) {
                ...
                if ((ev.pageX > bounds.left && ev.pageX < bounds.right) && (ev.pageY > bounds.top && ev.pageY < bounds.bottom)) return;
            }
    ...
    if (sessionId && userData) {
        return (
            <div className='user-dropdown'>
                ...
                < BiChevronDown id='dropdown-icon' className={`clickable-icons-${theme}`} style={iconStyle} onClick={() => setShowMenu(!showMenu)} />
                {showMenu &&
